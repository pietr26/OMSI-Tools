...
wdgTrip::wdgTrip(QWidget *parent, LFClientAPIInterface *api)
    : QWidget(parent)
    , ui(new Ui::wdgTrip),
    api(api)
{
    ...
    connect(api, &LFClientAPIInterface::loginStatusChanged, this, &wdgTrip::loadUi);
}
...
void wdgTrip::loadUi()
{
    if (api->isLoggedIn() && !tripData.isEmpty())
    {
    ...
    {
        trip = new QStandardItemModel();
        ui->tvwTrip->setModel(trip);

        ui->lblLineRoute->setText("-/-");
        ui->lblTrip->setText("---");
        ui->lblStopCount->setText("-");
    }
}
